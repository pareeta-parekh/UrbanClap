...
class showServiceView(APIView):
    
    # queryset = Appliedservice.objects.filter(id = 1) #get(token_id = token)
    serializer_class = APPSRSerializer
    
    # filter_backends = [filters.OrderingFilter]
    # ordering_fields = ['user_name', 'email']
    
    # search_fields = ['email']
    def get(self, request, id):
        queryset = Appliedservice.objects.filter(id = 1) #get(token_id = token)
        serializer_class = APPSRSerializer(queryset, many=True)
        #serializer = serializer_class(queryset, many=True)
        filter_bakends = [DjangoFilterBackend]
        filterset_fields = [ 'service_name']

        return Response({'data':serializer_class.data})
        # return render(request, 'serviceProvider/showServices.html', {'context':serializer.data})
...
#             filter_bakends = [DjangoFilterBackend]
#             filterset_fields = ['']
#             # return Response(spobj.services)
#             return render(request, 'serviceProvider/showServices.html', {'sprSR':spobj.services,'filter-backends': filter_bakends})
